...
export const handleImageUpload = async (
  ...
  if (!csrfToken) {
    // Attempt to refresh CSRF cookie if missing
    try {
      await fetch("/sanctum/csrf-cookie")
    } catch (e) {
      console.warn("Failed to refresh CSRF cookie", e)
    }
  }
...
export const handleAttachmentUpload = async (file: File): Promise<string> => {
  if (!file) {
    throw new Error("No file provided")
  }
  ...
  if (file.size > MAX_ATTACHMENT_SIZE) {
    throw new Error(
      `File size exceeds maximum allowed (${MAX_ATTACHMENT_SIZE / (1024 * 1024)}MB)`
    )
  }
  ...
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest()
    xhr.open("POST", "/api/v1/posts/attachments/upload")
